...
def analyzeFile(item):
    ...
    #added by MAZ- check if file already exists in output directory, skip it if so
    runFile = True
    #if we dont want to overwrite the file, we need to check if it already exists 
    if cfg.OVERWRITE==0:
        # We have to check if output path is a file or directory- borrowed from saving part below. If it's just one file, presumably you DO want to overwrite it, so not going to run this piece for that case
        if not cfg.OUTPUT_PATH.rsplit(".", 1)[-1].lower() in ["txt", "csv"]:
            ...
            rtype = ".birdnet.embeddings.txt"
            
            outfilepath = Path(rdir)
            #check if file exists and skip it if it does
            outfiles = list(outfilepath.glob('*'+rtype))
            matchlist = [rpath.split('/')[1].split('.')[0] in i.name for i in outfiles]
            matchuse = list(compress(outfiles,matchlist))
            if matchuse:
                runFile = False
                
    #print(f"runfile is {runFile}")  
    ...
    if runFile:
        ...
        print(f"File already exists in output folder. Skipping {fpath}.")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--overwrite", 
        type = int, 
        default = cfg.OVERWRITE, #default to overwriting
        help="Set this to 0 avoid overwriting files in output directory.",
    )
    ...
    #set overwrite-added by MAZ
    cfg.OVERWRITE = args.overwrite #get around boolean being frustrating
    ...
    # Set custom classifier?- added by MAZ 
    ...
    #read out number of species to check if it's using my classifier- added by MAZ 
